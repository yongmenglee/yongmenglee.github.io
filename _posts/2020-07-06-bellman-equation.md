...
In this case, there is only one associated next state $s'$ and reward $r$ for each action $a$. Therefore, the double sum in $\eqref{eq:sv_bellman}$ and the transition function $p(s',r|s,a) = 1$ is omitted.
