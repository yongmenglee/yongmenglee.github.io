...
In this case, there is only one associated next state $s'$ and reward $r$ for each action $a$. Therefore, the double sum and the transition function $p(s',r|s,a) = 1$ in $\eqref{eq:sv_bellman}$ is omitted.
